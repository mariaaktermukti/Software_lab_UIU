...
if ( isset($_GET["id"]) && isset($_GET["name"]) && isset($_GET["dept"]) ) {
    // Process update
    $id = $_GET["id"];
    $dept = $_GET["dept"];
    $name = $_GET["name"];
    ...
    mysqli_query( $connect, $query ) or die("Update failed: " . mysqli_error($connect));
    echo "Student updated! <br><a href='readStudent.php'>View all</a>";
    exit;
}

// Show form with existing data
$id = $_GET["id"];
$result = mysqli_query( $connect, "SELECT * FROM student WHERE id=$id" ) or die("Can't fetch");
